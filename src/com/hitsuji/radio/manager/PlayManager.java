...
implements OnBufferingUpdateListener, OnCompletionListener, OnErrorListener,OnPreparedListener
{
	...
	private final IPlayManagerApi.Stub mBindService = new IPlayManagerApi.Stub() {
		...
		public String loadCurrentContent() throws RemoteException {
			...
			track.loadContent(PlayManager.this.getFilesDir().getAbsolutePath());
		...
		@Override
		public String[] getImageUrl(int idx) throws RemoteException {
			// TODO Auto-generated method stub
			Track track = getTopTrack();
			if (track==null) return null;
		...
		try {
			if (action.equals(CREATE_PLAYLIST_ACTION)) {
				...
				if (!action.equals(mPrevAction) &&
					...
					int ret;
					try {
						ret = track.postLike(this.getFilesDir().getAbsolutePath());
					} catch (IOException e) {
					...
					Result result = checkResponse(this.getFilesDir().getAbsolutePath()+
				...
				if (!action.equals(mPrevAction) &&
					...
					int ret;
					try {
						ret = track.postBan(this.getFilesDir().getAbsolutePath());
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
						doToasting(e.toString());
	...
	private void clearAll() {
		...
		Track.clearAll(this.getFilesDir().getAbsolutePath());    	
		Auth.clearAll(this.getFilesDir().getAbsolutePath());
		Radio.clearAll(this.getFilesDir().getAbsolutePath());
	...
	private void createPlayList(int type, String similar, boolean fill){
		...
		if (radio instanceof Similar) {
			ret = ((Similar)radio).updateTrackList(path, null, fill);
		} else {
			ret = radio.updateTrackList(path, null);
		}
		if (ret!=0) {
	...
	private class LoadHandler extends Handler {
		...
		public void handleMessage(Message msg){
			if (msg.what == LOAD && msg.obj!=null){
				...
				track.fetchImageUrls(PlayManager.this.getFilesDir().getAbsolutePath());
				track.loadArtistInfo(PlayManager.this.getFilesDir().getAbsolutePath(), 
						Util.getName(PlayManager.this));
				track.loadSimilarArtists(PlayManager.this.getFilesDir().getAbsolutePath());
				
				if (Auth.getSessionkey(PlayManager.this.getFilesDir().getAbsolutePath()) != null)
					track.notifyNowPlaying(PlayManager.this.getFilesDir().getAbsolutePath());
