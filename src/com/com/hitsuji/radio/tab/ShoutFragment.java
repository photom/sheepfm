...
import com.hitsuji.radio.view.TextViewRoboto;
import com.util.Log;
...
public class ShoutFragment extends TabBaseFragment {
	...
	private class ShoutItemArrayAdapter extends ArrayAdapter<ShoutItem> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (item instanceof Header) {
				...
				TextView t = new TextViewRoboto(mParent);
				t.setText(h.getName());
				...
				TextView b;
				layout.removeAllViews();
				a = new TextViewRoboto(mParent);
				b = new TextViewRoboto(mParent);
				...
				content = content.replaceAll("\\[url=((http://|https://){1}[\\w\\.\\-/:\\#\\?\\=\\&\\;\\%\\~\\+\\(\\)]+)\\](.+?)\\[/url\\]",
						"<a href='$1'>$3</a>");
				content = content.replaceAll("\\[tag\\](.+?)\\[/tag\\]", 
						"<a href='"+"http://m.last.fm/tag/$1"+"'>$1</a>");
				content = content.replaceAll("\\[group\\](.+?)\\[/group\\]", 
						"<a href='"+"http://m.last.fm/group/$1"+"'>$1</a>");
				content = content.replaceAll("\\[artist\\](.+?)\\[/artist\\]", 
						"<a href='"+"http://m.last.fm/music/$1"+"'>$1</a>");
				content = content.replaceAll("\\[track artist=(.+?)\\](.+?)\\[/track\\]", 
						"<a href='"+"http://m.last.fm/music/$1/_/$2"+"'>$2</a>");

				b.setText(Html.fromHtml(String.format(fm, content)));
				b.setMovementMethod(LinkMovementMethod.getInstance());
				b.setLinkTextColor(Color.parseColor("#87CEFA"));
				b.setPadding(20, 5, 20, 20);
				Log.d(TAG, content);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle icicle){
		...
		mAdapter.add(new Header( "loading ..." ));
		return mBody;
	}
	...
	private class DisplayInitRunner implements Runnable{
		...
		public void run() {
			...
			if (!mTracks.isEmpty()){
				mAdapter.add(new Header( mTrack ));
			...
			if (!mArtists.isEmpty()){
				mAdapter.add(new Header( mArtist ));
