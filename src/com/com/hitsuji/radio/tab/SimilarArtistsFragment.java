...
public class SimilarArtistsFragment extends TabBaseFragment {
	...
	class Exec extends Handler{
		...
		private void setList(Bundle list){
	        ...
	        mAdapter.clear();
	        Log.d(TAG, "similarartistsfragment len:"+names.size());
	        for (int i=0; i<names.size(); i++) {
	        	Log.d(TAG, "similarartistsfragment name:"+names.get(i));
	...
	private class IntentItemArrayAdapter extends ArrayAdapter<Bundle> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Bitmap bitmap = BitmapFactory.decodeFile(
					mParent.getExternalFilesDir(null).getAbsolutePath() + File.separator +
					Track.SIMILAR_IMG_DIR + File.separator + idx);
			if (bitmap!=null){
				image.setImageBitmap(bitmap);
				Log.d(TAG, "bmph:"+bitmap.getHeight() + " m:"+match.getTextSize() + " n:"+name.getTextSize());
				//match.setHeight((int)(bitmap.getHeight()/1.5));
				//name.setHeight((int)(bitmap.getHeight()/1.5));
			}
